(function () {
  ...
  function setListPageSEO() {
    const title = '빛청모 | 성명공유';
    const description = '빛청모의 다양한 성명과 소식을 공유하는 공간입니다.';
    ...
    setMetaTag('keywords', '빛청모, 성소수자, 공조, 성명, 커뮤니티');
   ...
   */
  function setDetailPageSEO(statement) {
    if (!statement) {
      setListPageSEO();
      return;
    }
    ...
    const siteTitle = '빛청모 | 성명공유';
    const pageTitle = `${title} | ${siteTitle}`;
    const description = summary || body.substring(0, 160) || '빛청모의 성명을 확인하세요.';
    ...
    // 기본 메타 태그
    setMetaTag('description', description);
    setMetaTag('keywords', `빛청모, 성명, ${title}`);
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = ui.statements.seo;
  }
})();
