(function () {
  ...
  function createActivityDetail(activity) {
    ...
    return `
      <article class="card activity-detail" data-activity-slug="${safeSlug}" aria-labelledby="detail-title-${idSlug}">
        <header class="activity-detail-header">
          ...
          <h1 class="activity-detail-title" id="detail-title-${idSlug}">${escapeHtml(title)}</h1>
  ...
  function renderActivityDetail(activity, container) {
    ...
    const detailImg = containerEl.querySelector('.activity-detail-image');
    if (detailImg) {
      detailImg.addEventListener('error', function () {
        const wrap = detailImg.closest('.activity-image');
